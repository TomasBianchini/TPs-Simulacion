...
def fibonacci(capitalInicial =float('inf')):
    ...
    while i <= n:
        ...
        paridad = 'i'
        tirada = 1
        while (paridad != apostado and i !=n+1 and perdido + apuesta <= capitalInicial):
            i += 1 #acumulo un giro de ruleta
            valorGanador = tirarRuleta()
            flujoCaja.append(flujoCaja[-1] - apuesta) 
            flujoApuestas.append(apuesta)
            ...
            #     break
                ...
                tiradaGanadora.append(tirada)  #Guardamos tirada ganadora
                flujoCaja.append(flujoCaja[-1] + 2*apuesta)
                if(tirada == 1 or tirada == 2):
                    flujoApuestas.append(apuestaInicial) 
                else:
                    flujoApuestas.append(flujoApuestas[-2] + flujoApuestas[-1])
        # if(perdido + apuesta > capitalInicial): break
